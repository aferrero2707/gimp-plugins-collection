...
#brew reinstall \
#	little-cms2 openexr gettext intltool json-c \
#	json-glib glib-networking gexiv2 librsvg \
#	poppler gtk+ py2cairo pygtk gtk-mac-integration || exit 1
...
if [ ! -e babl ]; then
	...
	(cd babl && TIFF_LIBS="-ltiff -ljpeg -lz" JPEG_LIBS="-ljpeg" ./autogen.sh --disable-gtk-doc --enable-sse3=no --enable-sse4_1=no --enable-f16c=no --enable-altivec=no --prefix=${PREFIX} && make && make -j 3 install) >& babl.log || exit 1
fi

if [ ! -e gegl ]; then
	(git clone -b GEGL_0_4_8 https://git.gnome.org/browse/gegl) || exit 1
	(cd gegl && TIFF_LIBS="-ltiff -ljpeg -lz" JPEG_LIBS="-ljpeg" ./autogen.sh --disable-docs --prefix=${PREFIX} --enable-gtk-doc-html=no --enable-introspection=no && make -j 3 install) >& gegl.log || exit 1
fi
...
if [ ! -e gimp ]; then
	...
	(cd gimp && TIFF_LIBS="-ltiff -ljpeg -lz" JPEG_LIBS="-ljpeg" ./autogen.sh --disable-gtk-doc --disable-python --enable-sse=no --prefix=${PREFIX} && make -j 3 V=1 install) >& gimp.log || exit 1
